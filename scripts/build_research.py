...
def main() -> None:
    ...
    for path in post_paths:
        ...
        record = {"path": rel, "title": title, "date": date, "url": url, "body": body, "text": full_text, "outbound_links": urls}
        posts.append(record)
        if url:
    ...
    for post in posts:
        ...
        for entity in entities:
            aliases = [entity.get("name", "")] + list(entity.get("aliases") or [])
            hit = matches(text, aliases)
            if hit:
        ...
        for topic in topics:
            aliases = [topic.get("name", "")] + list(topic.get("aliases") or [])
            hit = matches(text, aliases)
            if hit:
        ...
        for url in post["outbound_links"]:
            ...
            if not target and "southallstories.uk" in normalized:
                ...
                target = canonical_lookup.get(parsed.path.rstrip("/"))
            ...
            item = source_urls.setdefault(url, {"url": url, "domain": domain, "posts": []})
            item["posts"].append(rel)
    ...
    edges = []
    for post in posts:
        nodes.append({"id": "post:" + post["path"], "type": "post", "label": post["title"], "path": post["path"], "date": post["date"]})
        for eid in post_entities.get(post["path"], []):
            edges.append({"source": "entity:" + eid, "target": "post:" + post["path"], "type": "mentioned-in"})
        for tid in post_topics.get(post["path"], []):
            edges.append({"source": "topic:" + tid, "target": "post:" + post["path"], "type": "mentioned-in"})
        for target in post_links.get(post["path"], []):
            edges.append({"source": "post:" + post["path"], "target": "post:" + target, "type": "links-to"})
    for entity in entities:
    ...
    for domain, paths in sorted(domain_posts.items()):
        ...
        for rel in sorted(paths):
            edges.append({"source": "post:" + rel, "target": "domain:" + domain, "type": "cites-domain"})
    ...
    summary = {
        ...
        "topics": len(topics),
        "entity_mentions": sum(len(v) for v in entity_mentions.values()),
        "topic_mentions": sum(len(v) for v in topic_mentions.values()),
        "internal_post_links": sum(len(v) for v in post_links.values()),
        "source_urls": len(source_urls),
        "source_domains": len(domain_posts),
